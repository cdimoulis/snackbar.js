  ...
  *********/
  ...
  var _setDom = function _setDom() {
    ...
    if (_body) {
      ...
      if (pre_dom_queue.length > 0) _flushQueue();
    }
  ...
  // message: text to display
  // opt: options to send
  var _addSnackbar = function _addSnackbar(message, opts) {
    var _this = this;
  ...
  // Remove a snackbar
  var _removeSnackbar = function _removeSnackbar(_el) {
  ...
  var _fadeIn = function _fadeIn(_el) {
    _changeOpacity(_el, 1, 500);
  };

  // Fade out individual snackbar
  var _fadeOut = function _fadeOut(_el, cb) {
  ...
  var _changeOpacity = function _changeOpacity(_el, value, time, cb) {
    // rate of change
    var fps = 24;
